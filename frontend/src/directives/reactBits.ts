...
/** 扩展 HTMLElement 以存储自定义事件处理函数引用 */
interface DirectiveEl extends HTMLElement {
  _spotlightMove?: (e: MouseEvent) => void
  _tiltMove?: (e: MouseEvent) => void
  _tiltLeave?: () => void
  _magnetMove?: (e: MouseEvent) => void
}
...
interface MagnetValue {
  maxDist?: number
  strength?: number
}

export const vSpotlight: ObjectDirective<DirectiveEl> = {
  mounted(el) {
  ...
  unmounted(el) {
    if (el._spotlightMove) {
      el.removeEventListener('mousemove', el._spotlightMove)
    }
  }
}

export const vTilt: ObjectDirective<DirectiveEl, TiltValue> = {
  mounted(el, binding: DirectiveBinding<TiltValue>) {
...
export const vMagnet: ObjectDirective<DirectiveEl, MagnetValue> = {
  mounted(el, binding: DirectiveBinding<MagnetValue>) {
