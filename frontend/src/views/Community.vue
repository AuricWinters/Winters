  ...
  <div class="community-page">
    ...
    <div class="community-container">
      ...
      <header class="community-header scroll-reveal">
        ...
        <div class="category-bar">
          <button
            ...
            :key="c.value"
            class="cat-btn"
            :class="{ active: store.category === c.value }"
      ...
      <div class="feed">
        ...
        <article
          v-if="pinnedPost"
          class="post-card pinned scroll-reveal"
          @click="$router.push('/community/' + pinnedPost.id)"
        >
          <div class="pin-badge">
            📌 {{ t('置顶') }}
          </div>
          <div class="post-header">
            <div class="author-avatar">
              {{ pinnedPost.author_name[0] }}
            </div>
            <div class="author-info">
              <div class="author-name">
                {{ pinnedPost.author_name }}
              </div>
              <div class="post-time">
                {{ pinnedPost.created_at }}
              </div>
            </div>
          </div>
          <h3 class="post-title">
          ...
          <div class="post-tags">
            <span
              v-for="tag in (Array.isArray(pinnedPost.tags) ? pinnedPost.tags : JSON.parse(pinnedPost.tags || '[]'))"
              :key="tag"
              class="post-tag"
            >#{{ tag }}</span>
          </div>
          <div class="post-actions">
            <button
              class="action-item"
              :class="{ liked: pinnedPost.liked }"
              @click="toggleLike(pinnedPost)"
            >
              <svg
                viewBox="0 0 24 24"
                :fill="pinnedPost.liked ? 'currentColor' : 'none'"
                stroke="currentColor"
                stroke-width="2"
        ...
        >
          ...
          <div class="post-tags">
            <span
              v-for="tag in (Array.isArray(post.tags) ? post.tags : JSON.parse(post.tags || '[]'))"
              :key="tag"
              class="post-tag"
            >#{{ tag }}</span>
          </div>
        ...
        <!-- 空状态 -->
        <div
          v-if="!store.loading && feedPosts.length === 0 && !store.tag && !store.category"
          class="empty-state"
        >
