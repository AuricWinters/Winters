...
/* ========== Mutable state (no reactivity needed) ========== */

let ctx: CanvasRenderingContext2D | null = null
let bounding: BoundingRect = { width: 0, height: 0, left: 0, top: 0 }
let noise: Noise
let lines: Point[][] = []
let mouse: Mouse = {
  x: -10,
...
/* ========== Config (read from props on each frame) ========== */
...
function drawLines() {
  ...
  ctx.beginPath()
  ctx.strokeStyle = getConfig().lineColor
...
function tick(t: number) {
  ...
  // Update CSS custom properties for the cursor dot
  const el = containerRef.value
  if (el) {
    el.style.setProperty('--x', `${mouse.sx}px`)
    el.style.setProperty('--y', `${mouse.sy}px`)
  }
