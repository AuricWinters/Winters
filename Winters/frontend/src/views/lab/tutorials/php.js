...
echo "年龄: $age 岁 <br>";
?>`,
            explanation: 'PHP 代码必须包含在 <?php ?> 标签中。echo 和 print 都用于输出，echo 更常用且可以输出多个值。PHP 变量都以 $ 开头，双引号字符串支持变量插值（解析变量），单引号则原样输出不解析变量。'
          },
          {
            title: '注释与字符串',
            code: `<?php
...
?>`,
            explanation: 'PHP 支持三种注释风格：//、#（单行）和 /* */（多行）。字符串有四种形式：单引号（不解析）、双引号（解析变量）、Heredoc（<<<，类似双引号多行）和 Nowdoc（<<<\'，类似单引号多行）。选择合适的字符串形式能避免意外解析和安全问题。'
          }
        ]
      },
      {
...
?>`,
            explanation: 'PHP 是弱类型语言但仍有类型系统：标量类型（int/float/string/boolean）、复合类型（array/object）和特殊类型（NULL/resource/callable）。gettype() 返回类型名，var_dump() 显示详细信息。使用 (类型)$var 可强制转换类型，空字符串、0、null、空数组转为布尔都是 false。'
          },
          {
            title: '索引数组与关联数组',
            code: `<?php
...
var_dump($mixed);
?>`,
            explanation: 'PHP 数组非常强大，实际上是有序映射（map）。索引数组用数字作为键（从0开始），关联数组用自定义字符串作为键。两者可以在同一数组中混合使用。=> 操作符将键映射到值。用 $arr[] = value 可自动追加元素。'
          },
          {
            title: '数组函数与遍历',
            code: `<?php
...
?>`,
            explanation: 'count() 获取长度；array_push() 尾部追加；array_merge() 合并数组；sort()/rsort() 排序；in_array() 判断成员。foreach 是最常用的遍历方式，可只取值 foreach($arr as $v) 或同时取键值 foreach($arr as $k=>$v)。这些数组函数是 PHP 开发的核心工具。'
          }
        ]
      },
      {
...
echo $result;
?>`,
            explanation: 'if-elseif-else 是最基本的条件结构。switch 用 break 防止穿透执行。match 是 PHP 8 引入的新特性，它是表达式（有返回值），不需要 break，语法更简洁，支持逗号分隔多个匹配值，default 处理默认情况。'
          },
          {
            title: '循环语句',
            code: `<?php
...
echo "总计: ¥{$total}";
?>`,
            explanation: 'for 适合已知循环次数的场景；while 先判断再执行可能一次都不运行；do-while 至少执行一次再判断。foreach 专门用于遍历数组和对象，可同时获取键和值，是 PHP 中最常用的循环方式。'
          },
          {
            title: '自定义函数',
            code: `<?php
...
echo $val; // 20
?>`,
            explanation: 'function 关键字定义函数，return 返回值。参数可设默认值 =value。PHP 7+ 支持 : type 声明返回值类型和参数类型，增强代码安全性。...$args 收集剩余参数为数组。&$param 使用引用传递，函数内修改会影响外部变量。'
          }
        ]
      },
      {
        ...
        title: '表单与超全局变量',
        icon: '🌐',
        description: '处理HTTP请求和表单数据',
        difficulty: 'beginner',
        examples: [
          {
            title: 'GET 与 POST 请求处理',
            code: `<?php
// ========== $_GET - 获取 URL 查询参数 ==========
...
$data = $_REQUEST["data"] ?? "";
?>`,
            explanation: '$_GET 收集 URL 问号后的查询参数（?key=value）。$_POST 收集 HTTP POST 请求体中的表单数据。?? 是空合并运算符，当值为 null 或未定义时使用默认值。$_SERVER["REQUEST_METHOD"] 可区分 GET 和 POST 请求。$_REQUEST 合并了三者但不推荐使用。'
          },
          {
            title: 'XSS 防护与 $_SERVER',
            code: `<?php
// ========== XSS 跨站脚本攻击防护 ==========
// ⚠️ 危险写法 - 直接输出用户输入
...
?>`,
            explanation: 'htmlspecialchars() 将 < > & \' " 等特殊字符转为 HTML 实体，防止 XSS 攻击。ENT_QUOTES 转换单双引号，UTF-8 指定编码。$_SERVER 提供服务器和请求环境信息：REQUEST_METHOD、REMOTE_ADDR（客户端IP）、HTTP_USER_AGENT（浏览器标识）、HTTP_X_REQUESTED_WITH（AJAX检测）等是开发中最常用的字段。'
          },
          {
            title: '文件上传基础',
            code: `<?php
...
?>`,
            explanation: '$_FILES 超全局变量存储上传文件信息，是一个二维数组，每个文件包含 name（原名）、tmp_name（临时路径）、size（大小）、type（MIME类型）、error（错误码）。处理流程：检查错误码 → 验证类型白名单 → 验证大小 → 生成唯一文件名 → move_uploaded_file() 移动到目标目录。注意表单必须设置 enctype="multipart/form-data"。'
          }
        ]
      },
      {
...
?>`,
            explanation: 'class 定义类，属性必须声明访问级别（public/private/protected）。__construct() 是构造方法，实例化时自动调用。$this-> 访问当前实例的属性和方法。private 属性通过 getter/setter 提供受控访问，可在 setter 中加入验证逻辑。PHP 7.4+ 支持属性类型声明如 public string $name。'
          },
          {
            title: '继承与 parent::',
            code: `<?php
...
$dog->bark();   // 旺财: 汪汪!（子类独有）
?>`,
            explanation: 'extends 让子类继承父类的所有公有和受保护的属性与方法。parent::__construct() 调用父类构造器完成初始化。子类可以重写（override）父类同名方法来改变行为。protected 成员对子类可见。继承实现了代码复用和多态——同一接口不同实现。'
          },
          {
            title: '命名空间与 use',
            code: `<?php
...
?>`,
            explanation: 'namespace 声明命名空间，组织代码避免类名冲突。命名空间用反斜杠 \\ 分隔层级（App\\Models）。use 关键字导入外部命名空间的类，可用 as 设置别名解决冲突。PHP 推荐遵循 PSR-4 自动加载规范：命名空间路径对应目录结构。大型项目必须使用命名空间管理代码。'
          }
        ]
      },
      {
...
$pdo = null;
?>`,
            explanation: 'PDO（PHP Data Objects）是 PHP 访问数据库的标准扩展，支持 MySQL、PostgreSQL、SQLite 等多种数据库。DSN 字符串指定数据库类型、主机、库名和字符集。推荐选项：ERRMODE_EXCEPTION 让错误抛异常而非静默失败；FETCH_ASSOC 返回关联数组（列名为键）；禁用模拟预处理使用真正的数据库预处理功能。'
          },
          {
            title: '预处理查询（SELECT）',
            code: `<?php
...
?>`,
            explanation: 'prepare() 编译 SQL 模板（带占位符），execute() 时才传入实际参数。这种预处理机制有效防止 SQL 注入攻击——参数始终被视为纯数据而非 SQL 代码。命名占位符 :name 比 ? 占位符更清晰。bindValue() 绑定时指定 PDO::PARAM_INT/PARAM_STR 类型。fetchAll() 一次性返回所有行，fetch() 逐行返回节省内存。'
          },
          {
            title: '完整 CRUD 操作',
            code: `<?php
...
?>`,
            explanation: '四大 CRUD 操作全部使用预处理语句保证安全：INSERT 插入后用 lastInsertId() 获取自增主ID；UPDATE/DELETE 后用 rowCount() 获取影响行数；事务（beginTransaction/commit/rollBack）确保多条 SQL 要么全成功要么全回滚，常用于金融转账等场景。所有参数都通过占位符绑定，杜绝 SQL 注入。'
          }
        ]
      },
      {
        id: 'php-project',
        title: '综合实战：简易博客后端API',
        icon: '🎯',
        description: 'RESTful风格的博客API实现',
        difficulty: 'advanced',
        examples: [
          {
            title: '路由设计与分发',
...
?>`,
            explanation: 'RESTful API 路由设计原则：资源用名词（articles），操作用 HTTP 方法（GET查询/POST创建/PUT更新/DELETE删除）。入口文件统一设置 JSON 响应头和 CORS 头。parse_url 分解 URI 路径提取资源和 ID。switch($method) 分发到对应的处理函数。jsonError() 统一返回错误格式。这是现代 PHP API 的标准架构模式。'
          },
          {
            title: '文章 CRUD 接口实现',
            code: `<?php
