  ...
  tutorials: [
      {
        ...
        title: 'Hello World',
        icon: '👋',
        description: '编写第一个Python程序，学习基本语法',
        difficulty: 'beginner',
        examples: [
          {
            title: 'print 输出与变量',
...
print(f"身高保留1位小数: {height:.1f}")`,
            explanation: 'Python的变量无需声明类型——解释器会自动推断。print()是最常用的输出函数。f-string（格式化字符串字面量）用f前缀+大括号{}插入变量和表达式，比旧的%格式化和.format()更简洁直观。'
          },
          {
            title: 'input 用户输入',
            code: `# input() 获取用户输入（始终返回字符串）
...
if len(data) >= 2:
    ...
    print(f"{n}, 今年{a}岁")`,
            explanation: 'input()函数暂停程序等待用户键盘输入，返回值永远是字符串str类型。需要数字时必须用int()或float()显式转换。split()方法可将一行输入按空白符拆分为列表。注意input中的提示文字要清晰友好。'
          },
          {
            title: '基本运算与类型查看',
            code: `# 算术运算
...
print(bool(1), bool("hi"))   # True True`,
            explanation: 'Python中//是整除（向下取整），%取余，**幂运算。type()可查看任意变量的类型。int()/float()/str()/bool()是常用的类型转换函数。注意0、空字符串""、空列表[]等在bool()转换时为False（称为falsy值）。'
          }
        ]
      },
      {
        ...
        title: '数据类型与容器',
        icon: '📦',
        description: 'Python丰富的内置数据结构',
        difficulty: 'beginner',
        examples: [
          {
            title: 'List 列表 - 有序可变序列',
            code: `# 创建列表
fruits = ["苹果", "香蕉", "橙子"]
...
print(nums)                   # [7,6,5,4,3,2,1,0]`,
            explanation: 'List是Python最常用的数据结构——有序、可变、允许重复元素。append/pop操作尾部O(1)，insert/remove中间操作O(n)。切片[start:end:step]极其强大，支持负索引从末尾计数。sort()原地排序不返回新列表，sorted()则返回新列表。'
          },
          {
            title: 'Dict 字典 - 键值对映射',
            code: `# 创建字典
...
print(squares)  # {1:1, 4:9, 9:16, 16:25, 25:36}`,
            explanation: 'Dict通过键快速查找值（平均O(1)），是Python最核心的数据结构之一。get()方法安全访问不存在的键。items()同时遍历键值对。Python 3.9+可用|运算符合并字典。字典推导式{key_expr: val_expr for ...}简洁地创建字典。'
          },
          {
            title: 'Set 集合 与 Tuple 元组',
            code: `# ===== Set 集合 - 无序不重复 =====
# 创建集合
...
print(f"最小:{mn} 最大:{mx} 平均:{avg:.1f}")`,
            explanation: 'Set用于去重和集合运算（交/并/差），成员判断O(1)极快。Tuple是不可变的有序序列，用圆括号创建（单元素需加逗号）。元组解包赋值是Python特色语法，广泛用于函数多返回值交换变量等场景。不可变性使元组可作为字典的键。'
          }
        ]
      },
      {
...
if x is None:
    print("x为空")  # 推荐用is判断None，而非==`,
            explanation: 'Python用缩进（通常4个空格）代替花括号来划分代码块——这是Python最显著的特征。elif相当于其他语言的else if。三元表达式value_if_true if condition else value_if_false。链式比较20<=temp<=30等价于20<=temp and temp<=30。判断None推荐用is而非==。'
          },
          {
            title: 'for / while 循环',
            code: `# for 循环 - 遍历可迭代对象
...
for i in range(3):
    ...
    print("没有触发break")  # 会执行`,
            explanation: 'for-in是Python最主要的循环结构，配合range/enumerate/zip使用非常强大。while适合不确定次数的循环。break跳出整个循环，continue跳过当前迭代。Python独有的for/while-else结构：循环正常完成（未被break中断）时执行else块，常用于搜索失败处理。'
          },
          {
            title: '列表推导式',
            code: `# 基本语法: [expression for item in iterable if condition]
...
print(sum(gen))  # 按需计算并求和`,
            explanation: '列表推导式是Python最具特色的语法糖之一，将map/filter/lambda组合成极简的可读形式。[expr for x in iter if cond]中expr是生成每个元素的公式，if可选用于过滤。还支持字典推导式{key:val for...}和集合推导式{expr for...}。生成器表达式用()代替[]，按需惰性计算节省大量内存。'
          }
        ]
      },
      {
...
def complex_func(a, b, *args, default=10, **kwargs):
    print(f"a={a}, b={b}, args={args}, default={default}, kwargs={kwargs}")`,
            explanation: 'def定义函数，return返回结果（无return时隐式返回None）。Python函数天然支持多返回值——实际上是返回元组并用解包接收。默认参数在调用时可不传。*args收集多余位置参数为元组，**kwargs收集多余关键字参数为字典。两者常一起使用实现灵活的函数接口。'
          },
          {
            title: 'Lambda 匿名函数',
            code: `# lambda语法: lambda 参数: 表达式
add = lambda a, b: a + b
print(add(3, 5))  # 8
...
# 复杂逻辑请用 def 定义具名函数`,
            explanation: 'lambda创建匿名函数，语法简洁但功能受限（单表达式）。最适合作为key=参数传给sorted/max/min等高阶函数，或配合map/filter做简单转换/过滤。对于复杂逻辑应优先使用def定义具名函数——更易读易调试。'
          },
          {
            title: '模块导入 import',
            code: `# 导入整个模块
...
print(p.resolve())            # 绝对路径`,
            explanation: 'import导入模块，Python拥有丰富的标准库（batteries included哲学）。math提供数学函数，random用于随机数，os/os/pathlib处理文件系统，json处理JSON数据。from X import Y只导入需要的名称减少内存占用。as给模块起别名避免命名冲突。应避免from X import *以免污染命名空间。'
          }
        ]
      },
      {
        ...
        title: '面向对象',
        icon: '📚',
        description: 'class类、继承和多态',
        difficulty: 'intermediate',
        examples: [
          {
            title: 'class 定义与 __init__',
            code: `# 定义类
class Dog:
    # 类属性（所有实例共享）
    species = "犬科动物"
...
dog1.color = "黄色"
print(dog1.color)            # 黄色`,
            explanation: 'class定义类，__init__是构造方法（不是构造函数），在实例创建时自动初始化属性。self指向当前实例（类似this），所有实例方法的第一个参数都是self。类属性属于类本身被所有实例共享，实例属性在__init__中通过self.xxx定义，每个实例独立拥有。Python允许运行时动态添加属性，体现了其动态语言的灵活性。'
          },
          {
            title: '继承 super() 与方法重写',
            code: `# 基类（父类）
class Animal:
...
print(Duck.__mro__)  # 方法解析顺序链`,
            explanation: 'class Child(Parent)实现继承，子类自动获得父类的所有属性和方法。super().__init__()调用父类构造函数确保正确初始化。子类可以重写override父类方法以定制行为。isinstance()检查对象是否是某类（或其子类）的实例。Python支持多继承，MRO(Method Resolution Order)决定方法查找顺序。继承遵循LSP原则——子类应能替换父类使用。'
          },
          {
            title: '@property 装饰器与特殊方法',
            code: `class Student:
...
print(v1[0], v1[1])  # 3 4  (__getitem__)`,
            explanation: '@property装饰器将方法转化为"计算属性"，访问时不用加括号，还可以定义setter实现赋值拦截。特殊方法（双下划线包围的__xxx__）让自定义类能像内置类型一样工作：__str__/__repr__控制字符串表示，__add__等实现运算符重载，__len__/__getitem__支持内置函数和索引操作。这些魔术方法是Python数据模型的核心。'
          }
        ]
      },
      {
        ...
        title: 'Pythonic 写法',
        icon: '🐍',
        description: '装饰器、上下文管理器和生成器',
        difficulty: 'intermediate',
        examples: [
          {
            title: '@ 装饰器',
            code: `# 装饰器本质是一个函数，它接收一个函数作为参数，
...
print(fibonacci(10))  # 第二次调用相同参数时会命中缓存`,
            explanation: '装饰器是Python的高级特性，@语法糖等价于func = decorator(func)。它能在不修改原函数代码的前提下增强功能——计时、日志、缓存、权限验证、重试等。带参数的装饰器需要多一层嵌套（工厂模式）：先调用外层函数返回真正的装饰器。functools.wraps可保留原函数的元信息（名称、文档字符串等）。'
          },
          {
            title: 'with 上下文管理器',
            code: `# with语句 - 自动管理资源（自动清理/关闭）
...
with tag("div"):
    print("内容")   # 输出: <div> 内容 <\/div>`,
            explanation: 'with语句确保资源在使用后被正确释放——文件关闭、锁释放、数据库连接归还等。底层协议要求对象实现__enter__（进入时调用，返回资源）和__exit__（离开时调用，负责清理）。即使with块内抛出异常，__exit__也会被执行。contextlib.contextmanager装饰器可以用yield简化上下文管理器的编写，yield之前是__enter__的逻辑，之后是__exit__的逻辑。'
          },
          {
            title: 'yield 生成器',
            code: `# 生成器 - 惰性计算的迭代器（节省内存）
...
#     process(line)`,
            explanation: '生成器是Python处理大数据流的利器。yield让函数变成生成器——每次调用next()执行到yield暂停返回值，下次调用从暂停处继续。优势：惰性计算（按需生成）、内存高效（一次只保存一个状态）、可表示无限序列。生成器表达式(x**2 for x in...)是列表推导式的惰性版本。yield from用于委托子生成器，简化嵌套yield。典型场景：读取大文件、流式数据处理、管道式处理链。'
          }
        ]
      },
      {
...
# "r+" - 读写模式
# "x" - 排他性创建（文件已存在则报错）`,
            explanation: 'open()打开文件返回文件对象，encoding="utf-8"指定编码避免中文乱码。"w"覆盖写入、"a"追加写入、"r"读取（默认）。强烈推荐搭配with语句使用——确保文件在任何情况下都会被正确关闭。read()全部读入内存适合小文件；直接迭代文件对象逐行处理适合大文件（内存高效）。二进制模式"rb"/"wb"用于图片音频等非文本文件。'
          },
          {
...
# RuntimeError - 通用运行时错误`,
            explanation: 'try-except捕获和处理异常，避免程序崩溃。except后可跟具体异常类型进行精准捕获，Exception基类兜底所有异常。else块在无异常时执行，finally块无论如何都执行（适合资源清理）。raise主动抛出异常，可自定义异常类继承Exception。assert用于调试时的前置条件检查（生产环境-O标志会禁用）。好的异常处理策略：精确捕获预期异常、记录日志、优雅降级或提示用户。'
          },
          {
            title: 'json 模块处理 JSON',
            code: `import json
...
print(json_str)  # {"__type__": "Person", "name": "小红", "age": 23}`,
            explanation: 'json模块是Python处理JSON数据的标配工具。dumps/dump负责序列化（Python→JSON），loads/load负责反序列化（JSON→Python）。ensure_ascii=False确保中文字符正常显示不被转义为\\uXXXX。indent参数美化输出便于阅读。JSON只支持有限的基本类型（dict/list/str/int/float/bool/None），自定义对象需要通过default参数提供转换函数或使用第三方库如orjson/pymarshal。'
          }
        ]
      },
      {
        id: 'py-project',
        title: '综合实战：命令行天气查询工具',
        icon: '🎯',
        description: '用requests+json实现天气查询CLI工具',
        difficulty: 'advanced',
        examples: [
          {
            title: 'requests.get 获取 API 数据',
...
for city in cities:
    ...
    print(f"{city:>12}: {temp_c}°C | {desc} | 湿度:{humidity}%")`,
            ...
            explanation: 'requests是Python最流行的HTTP库，API设计人性化。get(url)发送GET请求返回Response对象。response.json()自动将JSON响应体解析为Python字典/列表。raise_for_status()检查HTTP状态码，非2xx时抛异常。headers参数设置User-Agent等请求头（某些API要求）。timeout防止请求永久挂起。wttr.in是免费免注册的天气API，format=j1返回结构化JSON数据。'
          },
          {
            title: '解析 JSON 提取天气信息',
            code: `import requests
import json
...
display_weather("北京", parsed)`,
            ...
            explanation: '这段代码展示了完整的API数据提取流程：fetch_weather()负责网络请求和错误处理，parse_weather()负责从复杂的嵌套JSON结构中提取所需字段（使用get()方法配合默认值防KeyError），display_weather()负责格式化输出。JSON数据通常有多层嵌套结构，用get("key",[{}])[0]的模式安全地深入访问嵌套字段。将职责分离为三个函数使代码清晰易维护。'
          },
          {
            title: 'argparse 命令行参数与交互菜单',
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        interactive_mode()`,
            ...
            explanation: 'argparse是Python标准库的命令行解析神器。ArgumentParser定义参数说明，add_argument()添加位置参数（必填）和可选参数（--flag）。choices限制取值范围，action="store_true"定义布尔开关。sys.argv获取原始命令行参数列表。交互模式用while True循环+input()构建菜单系统，每轮show_menu()展示选项，根据用户输入分支处理。if __name__ == "__main__"是Python惯用法——确保脚本被直接运行时才执行入口代码，被import时不会触发。两种模式共存：有命令行参数走参数模式，否则进交互菜单。'
          }
        ]
      }
    ]
