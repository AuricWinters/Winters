...
console.log(name, age, isStudent);`,
            explanation: 'TypeScript 的基础类型包括 string（字符串）、number（数字，包含整数和浮点数）和 boolean（布尔值）。使用冒号 : 进行类型注解，让编译器在编码阶段就能发现类型错误。'
          },
          {
            title: '数组类型',
            code: `// 方式一：类型后加方括号 - 元素全是字符串
...
console.log(scores.length); // 3`,
            explanation: 'TypeScript 中有两种方式定义数组类型：string[] 和 Array<number>，两者完全等价。数组中的所有元素必须是同一类型，这比 JavaScript 更安全。'
          },
          {
            title: '特殊类型',
            code: `// 字面量类型 - 只能是特定值
...
let notDefined: undefined = undefined;`,
            explanation: '字面量类型限制变量只能是特定的值；any 可以绕过所有类型检查但会丧失 TS 的优势；unknown 是更安全的 any；never 表示永远不会正常返回；void 用于无返回值的函数；null 和 undefined 是各自的独立类型。'
          }
        ]
      },
      {
        ...
        title: '高级类型',
        icon: '📦',
        description: 'interface/type/联合/交叉/泛型入门',
        difficulty: 'beginner',
        examples: [
          {
            title: 'Interface 定义对象形状',
            code: `// 定义用户接口 - 描述对象的形状
interface User {
    name: string;
    age: number;
...
const admin: Admin = {
    ...
    permissions: ["read", "write", "delete"]
            ...
            explanation: 'interface 用于定义对象的结构（形状）。? 标记可选属性，readonly 标记只读属性。extends 关键字可以继承并扩展现有接口，实现代码复用。'
          },
          {
            title: 'Type 别名与联合/交叉类型',
            code: `// Type 别名 - 给类型起名字
...
let currentTheme: Theme = "dark";`,
            explanation: 'type 关键字用于创建类型别名，让复杂类型可复用。联合类型 | 表示"或"，值可以是列出的任一类型。交叉类型 & 表示"且"，合并多个类型的属性。联合字面量常用于模拟枚举。'
          },
          {
            title: '泛型入门',
            code: `// 泛型函数 - <T> 是类型参数，像函数参数一样传入类型
...
let idEntry = pair(1, "admin");   // [number, string]`,
            explanation: '泛型（Generics）让代码可以处理多种类型而不失类型安全。<T> 是类型变量，使用时才确定具体类型。泛型使函数、接口、类具备更好的复用性——一套代码适用于 string、number 等任意类型。'
          }
        ]
      },
      {
...
let myFunc: (x: number, y: number) => number = add;`,
            explanation: '每个参数都可以标注类型，返回值用 : 类型标注。void 表示没有返回值。箭头函数同样支持完整类型注解。函数变量的类型写成 (参数列表) => 返回值 的形式。'
          },
          {
            title: '可选参数与默认参数',
            code: `// 可选参数 - 加 ?，必须放在必需参数后面
...
sum(1, 2, 3, 4, 5); // 15`,
            explanation: '可选参数 ? 允许调用时不传该参数，且必须在必选参数之后。默认参数 = 在未传参时使用预设值。剩余参数 ...args 将多余参数收集为数组，常用于处理不定数量参数的场景。'
          },
          {
            title: '函数重载',
            code: `// 重载签名 - 先声明多个函数签名
...
fetchData("/api/users", (data) => {
    console.log(data); // data 自动推断为 string
            ...
            explanation: '函数重载允许同一函数名有不同参数/返回值类型组合。先写重载签名（只有类型），再写实现。回调函数的参数类型也要标注，这样在回调内就有完整的类型提示和检查。'
          }
        ]
      },
      {
        ...
        title: '类与修饰符',
        icon: '🔀',
        description: 'class的访问控制和readonly',
...
// console.log(p.email);    // ❌ Error! protected`,
            explanation: 'public（公开）、private（私有）、protected（受保护）是三种访问级别。public 默认可省略；private 仅限类内部使用；protected 允许子类访问。合理使用修饰符能封装内部细节，提高代码安全性。'
          },
          {
            title: '抽象类与接口实现',
            code: `// 抽象类 - 不能直接实例化，必须被继承
...
class Circle implements Drawable {
    ...
    draw(): void {
        console.log('绘制半径为 \${this.radius} 的圆');
            ...
            explanation: 'abstract 抽象类定义通用模板，abstract 方法强制子类实现。implements 让类遵循接口契约，必须实现接口中声明的所有方法。抽象类适合"is-a"关系，接口适合"can-do"能力描述。'
          },
          {
            title: '存取器 get/set 与 readonly',
...
// account.accountNumber = "xxx"; // ❌ readonly 不能修改`,
            explanation: 'get/set 存取器允许控制属性的读写行为，可在 getter 中计算派生值，在 setter 中加入验证逻辑。readonly 修饰符确保属性一旦初始化就不能被外部修改，常用于 ID 等不应变化的字段。'
          }
        ]
      },
      {
        ...
        title: '工具类型',
        icon: '📚',
        description: 'Partial/Pick/Omit/Record等内置工具类型',
        difficulty: 'intermediate',
        examples: [
          {
...
// todo.title = "xxx"; // ❌ 无法赋值，因为 readonly`,
            explanation: 'Partial<T> 将所有属性变为可选，非常适合更新操作（PATCH）。Required<T> 则相反，将所有属性设为必需。Readonly<T> 让所有属性变成只读，防止意外修改。这些都是 TypeScript 内置的工具类型。'
          },
          {
...
const permissions: RolePermissions = {
    ...
    viewer:   ["read"]
            ...
            explanation: 'Pick 从类型中挑选部分属性组成新类型，适合 API 响应精简。Omit 排除不需要的字段，常用于表单输入类型。Record<K, V> 快速创建对象映射类型，键是 K 类型，值是 V 类型。'
          },
          {
            title: 'Exclude / Extract / ReturnType / Parameters',
            code: `type Theme = "light" | "dark" | "auto" | "system";
...
// 结果: [url: string, options: { cache?: boolean }]`,
            explanation: 'Exclude 从联合类型中移除特定成员，Extract 则保留交集成员。ReturnType 和 Parameters 分别提取函数的返回值类型和参数类型元组，在编写高阶函数时非常有用。这些工具类型大大减少了手动重复定义的工作量。'
          }
        ]
      },
      {
        ...
        title: '泛型进阶',
        icon: '🔄',
        description: '泛型约束、条件类型和infer',
        difficulty: 'intermediate',
        examples: [
          {
            title: '泛型约束 extends',
            code: `// extends 约束泛型必须具有某些属性/能力
...
// kennel.add({ name: "猫" }); // ❌ 不是 Dog 类型`,
            explanation: 'extends 不仅用于类继承，还可约束泛型参数。T extends { length: number } 要求 T 必须拥有 length 属性（number 类型）。这样在函数体内就可以安全地访问 .length，编译器不会报错。'
          },
          {
            title: '条件类型',
            code: `// 条件类型: T extends U ? X : Y （类似三元表达式）
...
type T3 = TypeName<() => void>; // "函数"`,
            explanation: '条件类型根据类型关系选择不同结果，语法类似三元运算符。T extends U ? X : Y 判断 T 是否兼容 U。嵌套条件类型可实现复杂的类型判断分支，常用于类型工具库的实现。'
          },
          {
            title: 'infer 关键字与映射类型',
            code: `// infer - 在条件类型中推断类型变量
...
type ReadonlyAndNullable<T> = {
    readonly [K in keyof T]: T[K] | null;
            ...
            explanation: 'infer 在条件类型中声明类型变量并自动推断其具体类型，常用于从复杂类型中提取内部结构。映射类型用 [K in keyof T] 遍历对象的所有键，配合 readonly、? 等修饰符可批量转换属性特征，这是许多内置工具类型的底层原理。'
          }
        ]
      },
      {
        id: 'ts-project',
        title: '综合实战：类型安全的API请求封装',
        icon: '🎯',
        description: '用泛型封装axios请求，实现完整类型安全',
        difficulty: 'advanced',
        examples: [
          {
            title: '定义泛型响应接口',
            code: `// 统一的API响应格式 - 泛型T代表实际数据
...
interface PageParams {
    ...
    keyword?: string;
            ...
            explanation: 'ApiResponse<T> 是核心泛型接口，code/message/timestamp 固定不变，而 data 的类型通过泛型 T 动态决定。这样无论是用户列表、文章详情还是其他业务数据，都能复用统一的响应结构并获得完整的类型推导。'
          },
          {
            title: '封装类型安全的请求函数',
...
async function post<T>(url: string, data: unknown): Promise<ApiResponse<T>> {
    return apiRequest<T>('POST', url, data);
            ...
            explanation: 'apiRequest<T> 是核心泛型函数，T 决定返回的 data 字段类型。get<T> 和 post<T> 进一步简化调用。axios 拦截器统一处理 token 注入和错误跳转。调用时只需指定 T，整个请求-响应链路都有完整的类型保护。'
          },
          {
            title: '完整使用示例',
            code: `// ========== 获取用户列表 ==========
...
createArticle({
    ...
    tags: ["前端", "TypeScript"]
            ...
            explanation: '这就是泛型的威力所在：get<UserListResponse> 调用后，res.data 自动获得 UserListResponse 类型，res.data.list[i].name 有完整补全。post<Article> 同理。如果接口返回的数据结构与类型不符，TypeScript 会立即报错，将运行时 bug 消灭在编码阶段。Omit<Article, \'id\' | \'author\'> 确保创建文章时不会误传应由后端生成的字段。'
          }
        ]
      }
    ]
