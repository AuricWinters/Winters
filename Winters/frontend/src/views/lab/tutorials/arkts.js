  ...
  tutorials: [
    {
      ...
      title: 'Hello World',
      icon: '👋',
      description: 'ArkTS基础，学习声明式UI',
      difficulty: 'beginner',
      examples: [
        {
          title: '基本组件结构',
          code: `// @Entry - 装饰器：标记页面入口组件
...
struct Index {
    ...
    build() {
        ...
        .height('100%')               // Row 占满屏幕高度
          ...
          explanation: 'ArkTS 是 HarmonyOS 的声明式 UI 框架。@Entry 标记页面入口，@Component 标记自定义组件，struct 定义组件结构体。build() 方法内用声明式语法描述 UI 层级：Row(行)和Column(列)是基础布局容器，Text 显示文本。.fontSize()、.fontWeight() 是链式 API 设置样式属性。'
        },
        {
          title: '常用基础组件',
          code: `@Entry
...
struct DemoPage {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: 'ArkTS 提供丰富的内置组件：Text 文本、Image 图片、Button 按钮、Toggle 开关等。每个组件通过链式 API 设置属性（.width/.height/.fontSize/.margin 等）。Button 的 onClick 回调处理点击事件，Toggle 的 onChange 回调处理状态切换。@State 变量变化会自动驱动 UI 更新。'
        },
        {
          title: '样式与布局属性',
          code: `@Entry
...
struct StyleDemo {
    build() {
        ...
        .padding(20)
          ...
          explanation: 'ArkTS 中尺寸单位使用 vp（虚拟像素），自动适配不同屏幕密度。padding 控制内容区内边距，margin 控制外边距。border 设置边框样式，borderRadius 设圆角。Row/Column 通过 justifyContent/alignItems 控制子元素对齐方式。layoutWeight 实现弹性布局按比例分配空间。'
        }
      ]
    },
    {
      ...
      title: '状态管理',
      icon: '📦',
      description: '@State/@Prop/@Link/@Provide/@Consume等装饰器',
      difficulty: 'beginner',
      examples: [
        {
          title: '@State 组件内部状态',
...
struct CounterPage {
    ...
    build() {
        ...
        .padding(30)
          ...
          explanation: '@State 是最基础的状态装饰器，用于组件内部的响应式变量。当 @State 变量被修改时，所有依赖它的 UI 节点会自动重新渲染。这是 ArkTS 响应式系统的核心机制——数据驱动视图更新，无需手动操作 DOM。if 条件渲染根据状态值动态显示/隐藏组件。'
        },
        {
          title: '@Prop 父子单向数据传递',
          code: `// ========== 子组件 - 接收父组件传入的数据 ==========
...
struct PropDemoPage {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: '@Prop 实现父子组件间的单向数据流：父组件通过属性传递数据给子组件，子组件只能读取不能直接修改。这种单向数据流使数据流向清晰可追踪。ForEach 用于循环渲染列表，将数组中的每项数据传递给子组件实例。每个子组件接收独立的 prop 值副本。'
        },
        {
          title: '@Link 双向绑定 & @Provide/@Consume 跨层级',
          code: `// ========== @Link 双向绑定 ==========
@Component
struct ScoreEditor {
    // @Link - 双向数据绑定
...
struct StateAdvancedDemo {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: '@Link 实现父子间双向绑定：用 $score 语法创建链接，子组件修改值会同步回父组件。@Provide/@Consume 解决跨层级数据共享问题：祖先组件用 @Provide 提供数据，任意层级的后代用 @Consume 直接消费，无需层层传递 props。这三种装饰器覆盖了 ArkTS 状态管理的核心场景。'
        }
      ]
    },
    {
...
struct ConditionalRender {
    ...
    build() {
        ...
        .alignItems(HorizontalAlign.Center)
          ...
          explanation: 'If/Else 是 ArkTS 的条件渲染机制，根据状态值决定渲染哪个分支的 UI。与 show/hide 不同的是，If 为 false 时组件不会创建（节省资源），而 hide 只是隐藏但仍然占用空间。支持嵌套 If 判断复杂条件。条件变化时会销毁旧分支组件并创建新分支组件。'
        },
        {
          title: 'ForEach 列表渲染',
...
struct ListRenderDemo {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: 'ForEach 是 ArkTS 的列表渲染指令，三个参数分别是：数据源数组、item→UI 的生成函数、key 生成器函数。key 生成器必须返回稳定唯一的字符串，用于高效地追踪列表项变化（增删改排序）。ListItem 是 List 的专用子项容器。filter/map/reduce 等 JS 数组方法在 ArkTS 中同样可用。'
        },
        {
          title: 'show/hide 与可见性控制',
          code: `@Entry
@Component
struct VisibilityDemo {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: '控制组件可见性有两种方式：① visibility(Visible/Hidden/None) — Hidden 时组件不可见但仍占布局空间，None 时不占空间；② If 条件渲染 — 条件为 false 时根本不创建组件节点，适合不需要的内容（如加载状态）。LoadingProgress 是系统内置的加载动画组件。aboutToAppear 是组件生命周期钩子，在组件即将出现时调用。'
        }
      ]
    },
    {
      ...
      title: '自定义组件',
      icon: '⚙️',
      description: '@Builder/@Styles/@Extend复用UI代码',
      difficulty: 'intermediate',
      examples: [
        {
          title: '@Builder 轻量复用函数',
          code: `@Entry
@Component
struct BuilderDemo {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: '@Builder 装饰器将一段 UI 代码封装成可复用的轻量函数。可以在同一组件内多次调用，传入不同参数生成略有差异的 UI。适合封装重复的小型 UI 片段（如列表项、卡片、标签）。注意 @Builder 仅限于当前组件内使用，如果需要在多个组件间复用应使用独立 struct 组件或 @Styles/@Extend。'
        },
        {
          title: '@Styles 抽取通用样式',
          code: `@Entry
...
struct StylesDemo {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: '@Styles 将一组通用样式属性封装成可复用的样式函数。定义后可在当前组件中任何元素上通过 .styleName() 链式调用。好处是：统一管理样式确保一致性、减少重复代码、修改一处全局生效。@Styles 只能封装通用属性（不含事件回调）。如果需要扩展特定组件的独有属性，使用 @Extend。'
        },
        {
          title: '@Extend 扩展原生组件 & 自定义 struct 组件',
          code: `// ========== @Extend - 扩展特定组件类型的样式 ==========
// 只能作用于指定类型（这里是 Text）
@Extend(Text)
function fancyText() {
    .fontSize(16)
...
struct ComponentDemo {
    build() {
        ...
        .padding(20)
          ...
          explanation: '@Extend 扩展特定原生组件（如 Text/Button）的样式能力，可在全局任意位置使用，不受组件边界限制。自定义 struct + @Component 创建真正的独立可复用组件，有私有属性和完整生命周期。ProductCard 示例展示了如何构建带图片、名称、价格的电商卡片组件。组合使用 @Extend（细粒度样式）+ @Component（结构化组件）是 ArkTS 复用的最佳实践。'
        }
      ]
    },
    {
      ...
      title: '页面路由与导航',
      icon: '📚',
      description: 'router.push/router.back页面跳转',
      difficulty: 'intermediate',
      examples: [
        {
          title: '页面跳转 router.pushUrl',
...
struct HomePage {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: 'router.pushUrl() 将新页面压入导航栈，保留当前页面以便返回。url 参数指向目标页面路径（需在 main_pages.json 中注册）。params 对象用于向目标页传递数据，可以是任意 JSON 兼容的对象。pushUrl 返回 Promise，可用 then/catch 处理成功和失败。这是 ArkTS 中最常用的页面导航方式。'
        },
        {
          title: '返回上一页与参数获取',
          code: `import router from '@ohos.router'
...
struct DetailPage {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: 'router.getParams() 在目标页面获取来源页面传递的参数，通常在 aboutToAppear 生命周期中调用。router.back() 返回上一页（弹出栈顶页面），可选携带返回数据。router.replaceUrl() 替换当前页面而非压栈，适用于登录后跳转主页等不需要返回的场景。路由参数传递基于深拷贝，修改不会影响原页面数据。'
        },
        {
          title: '路由配置 main_pages.json',
          code: `/* ========================================
...
struct RouterConfigDemo {
    ...
    build() {
        ...
        .padding(20)
          ...
          explanation: 'main_pages.json 是路由注册表，所有可导航页面必须在 src 数组中声明路径。RouterMode.Standard（默认）每次 push 创建新页面实例；RouterMode.Single 则检查栈中是否已有同 URL 页面，有则复用（带回栈顶），避免重复创建。router.getLength() 获取栈深度，getIndex() 获取当前位置，clear() 清空整个栈。正确配置路由模式能有效管理内存和用户体验。'
        }
      ]
    },
    {
      ...
      title: '综合实战：待办事项App',
      icon: '🎯',
      description: '完整HarmonyOS待办事项应用',
      difficulty: 'advanced',
      examples: [
        {
          title: 'TodoItem 自定义组件',
          code: `// 定义待办事项数据接口
interface TodoData {
    id: number
    text: string
...
struct TodoItem {
    ...
    build() {
        ...
        .shadow({ radius: 2, color: '#00000008' })
          ...
          explanation: 'TodoItem 是高度封装的自定义组件，展示单条待办事项。接收 todo 数据对象和两个回调函数（onToggle 切换完成状态、onDelete 删除）。UI 包含：左侧状态切换按钮（✅/⬜）、中间文本区（支持删除线效果和时间戳）、右侧删除按钮。使用 ?. 可选链安全调用可能未传入的回调。maxLines + textOverflow 实现文本溢出省略。'
        },
        {
          title: 'TodoList 主页面完整实现',
          code: `import router from '@ohos.router'
...
struct TodoListApp {
    ...
    FilterTab(label: string, mode: string, count: number) {
        Text('\${label} (\${count})')
          ...
          explanation: '这是一个完整的待办事项应用主页面。核心功能包括：@StorageLink 实现数据持久化到 AppStorage（应用重启后数据仍在）；计算属性 filteredTodos 根据过滤模式动态筛选；addTodo/toggleTodo/deleteTodo/clearCompleted 四大操作方法；TextInput 输入框配合回车提交和按钮提交双触发；FilterTab @Builder 构建可复用的过滤标签；空状态友好提示；底部清除已完成项工具栏。整体展示了 ArkTS 状态管理、列表渲染、条件渲染、自定义组件、@Builder 复用的综合运用。'
        },
        {
          title: '完整数据流与交互逻辑',
          code: `/* ==============================================
...
*/`,
          explanation: '这个综合实战展示了完整的 ArkTS 应用开发流程。核心设计原则：① @StorageLink 实现数据持久化和跨页面共享；② 不可变更新（创建新数组赋值）确保响应式系统正常工作；③ 合理拆分组件（TodoItem 独立组件 + FilterTab/StatCard @Builder）；④ ForEach 配合稳定的 key 生成器保证列表渲染性能。StatsPage 展示了如何从同一个 @StorageLink 数据源读取并展示统计信息，体现了 ArkTS 单向数据流的优雅之处——数据改变自动传播到所有订阅者。'
        }
      ]
    }
  ]
