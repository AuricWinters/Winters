...
### Added
- 84 组件画廊 — React Bits 全量搬迁到项目
- 组件实验室 `/lab/showcase` 7 个子页（文字/悬停/入场/背景/UI/3D）
- 动效槽位系统 — 9 个槽位（标题/卡片/按钮/背景/文字/点击/页面/导航），Pinia + localStorage 持久化
- SettingsPanel 支持动效实时切换
...
- 代码执行引擎（6 语言：C/Java/JS/TS/Python/PHP，subprocess + xterm.js）
...
### Changed
- React Bits 风格 UI 重设计（24 交互 demo + 135 组件分类）
- AI 自执行施工计划（4 阶段 40+ 任务清单）
- Phase 0: CSS 基础设施统一（补变量/建 card 基类/动画提升/颜色清零）
- Phase 1: Easy 动效 9 件套（Vue Composables + Components）
- Phase 2: Medium 组件 10 件（Canvas/IO/SVG 动效）
- Phase 3: 全局 card-spotlight 覆盖（App/Header/Home → Community/Lab → Blog/Settings/Login）
